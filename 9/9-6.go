...
func EventSource(url string) (chan Event, context.Context, error) {
	...
	ctx, channel := context.WithCancel(req.Context())
	...
	go receiveSSE(events, channel, res)
	return events, ctx, nil
}

func receiveSSE(events chan Event, channel context.CancelFunc, res *http.Response) {
	...
	for {
		select {
			case <-ctx.Done();
				return
		case event := <-events:
			fmt.Printf("Event(ID=5s, Event=%s): %s\n", event.ID, event.Name, event.Data)
		}
	}
}